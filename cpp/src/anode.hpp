...
namespace Turnip {
template<typename T>
class ANode : public std::enable_shared_from_this<T>{
 public:
    ...
    std::weak_ptr<T> Parent() const { return m_Parent.lock(); }
    ...
    int Order() const {
        ...
        auto it = std::find_if(siblings.begin(), siblings.end(),
            [this](const std::shared_ptr<T>& node) {
...
template<typename T>
std::size_t ANode<T>::m_UIDCount = 0;
