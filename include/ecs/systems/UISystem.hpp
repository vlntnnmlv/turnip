...
private:
        ...
        for (Axis axis : {Axis::HORIZONTAL, Axis::VERTICAL}) {
          ...
          case turnip::SizeType::END:
            pos =
                AxisHelper::GetRectSize(transformComponent->rect, axis) -
                AxisHelper::GetPadding(layoutComponent->padding, axis, false) -
                AxisHelper::GetMargin(childLayoutComponent->margin, axis,
                                      false) -
                AxisHelper::GetRectSize(childTransformComponent->rect, axis);
            break;
          case turnip::SizeType::CENTER:
            pos =
                AxisHelper::GetRectPosition(transformComponent->rect, axis) +
  ...
  void MeasureAxis(EntityID _EntityID, ChildrenComponent *_ChildrenComponent) {
    ...
    StackComponent *stackComponent =
        m_Registry.GetComponent<StackComponent>(_EntityID);

    Axis axis = stackComponent->type == StackType::HORIZONTAL ? Axis::HORIZONTAL
                                                              : Axis::VERTICAL;
    ...
    for (auto const &child : _ChildrenComponent->children) {
      ...
      if (AxisHelper::GetLayoutSizeType(childLayoutComponent->size, axis) !=
          SizeType::FILL) {
        fillSpace -=
            AxisHelper::GetLayoutSize(childLayoutComponent->size, axis);
      } else
    ...
    for (auto const &child : _ChildrenComponent->children) {
      ...
      float orthoAvailable =
          ...
          AxisHelper::GetPadding(layoutComponent->padding, orthogonalAxis,
                                 true) -
          AxisHelper::GetPadding(layoutComponent->padding, orthogonalAxis,
                                 false) -
          AxisHelper::GetMargin(childLayoutComponent->margin, orthogonalAxis,
                                true) -
          AxisHelper::GetMargin(childLayoutComponent->margin, orthogonalAxis,
                                false);

      switch (AxisHelper::GetLayoutSizeType(childLayoutComponent->size,
                                            orthogonalAxis)) {
      case SizeType::FILL:
      ...
      default:
        AxisHelper::GetRectSize(childTransformComponent->rect, orthogonalAxis) =
            AxisHelper::GetLayoutSize(childLayoutComponent->size,
                                      orthogonalAxis);
        break;
      }
    }
  ...
  void ArrangeAxis(EntityID _EntityID, ChildrenComponent *_ChildrenComponent) {
    ...
    for (auto const &child : _ChildrenComponent->children) {
      ...
      case SizeType::FILL:
      case SizeType::START:
        offset = AxisHelper::GetPadding(layoutComponent->padding, axis, true);
        break;
      case SizeType::END:
        offset =
            -AxisHelper::GetPadding(layoutComponent->padding, axis, false) -
      ...
      float position =
          offset +
          AxisHelper::GetMargin(childLayoutComponent->margin, axis, true) +
          GetChildOrder(child, _ChildrenComponent->children) *
              stackComponent->spacing;

      for (auto const &otherChild : _ChildrenComponent->children) {
        TransformComponent *otherChildTransformComponent =
            m_Registry.GetComponent<TransformComponent>(otherChild);

        LayoutComponent *otherChildLayoutComponent =
            m_Registry.GetComponent<LayoutComponent>(otherChild);

        if (GetChildOrder(otherChild, _ChildrenComponent->children) <
            GetChildOrder(child, _ChildrenComponent->children)) {
          position += AxisHelper::GetRectSize(
                          otherChildTransformComponent->rect, axis) +
                      AxisHelper::GetMargin(otherChildLayoutComponent->margin,
                                            axis, true) +
                      AxisHelper::GetMargin(otherChildLayoutComponent->margin,
                                            axis, false);
        }
      ...
      float &pos = AxisHelper::GetRectPosition(childTransformComponent->rect,
                                               orthogonalAxis);
      float orthoSize = AxisHelper::GetRectSize(childTransformComponent->rect,
                                                orthogonalAxis);
      ...
      case SizeType::FILL:
      case SizeType::START:
        pos = AxisHelper::GetPadding(layoutComponent->padding, orthogonalAxis,
                                     true) +
              AxisHelper::GetMargin(childLayoutComponent->margin,
                                    orthogonalAxis, true);
        break;
      case SizeType::END:
        pos =
      ...
      case SizeType::ABSOLUTE:
        pos = (orthogonalAxis == Axis::HORIZONTAL)
                  ? childLayoutComponent->size.x
                  : childLayoutComponent->size.y;
        break;
      }
    }
  ...
  int GetChildOrder(EntityID _Child, std::vector<EntityID> _Children) {
    ...
    return (it != _Children.end())
               ? static_cast<int>(std::distance(_Children.begin(), it))
               : 0;
  }
};
} // namespace turnip::ecs
