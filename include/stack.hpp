...
public:
    ...
    Stack(const std::string &_ID, StackType _Type, ContentType _ContentType, turnip::Size _Size,
          ::Color _Color)
        : Node(_ID, _Size, _Color), m_Type(_Type), m_ContentType(_ContentType) {}

    static std::shared_ptr<Stack>
    Create(const std::string &_ID, const std::shared_ptr<Node> &_Parent, StackType _Type,
           ContentType _ContentType = ContentType::CENTER,
           turnip::Size _Size = turnip::Size{turnip::SizeType::FILL, turnip::SizeType::FILL},
           ::Color _Color = {255, 255, 255, 255}) {
        auto stack = std::make_shared<Stack>(_ID, _Type, _ContentType, _Size, _Color);
    ...
    void Measure() override {
        MeasureAxis(m_Type == StackType::VERTICAL ? Axis::VERTICAL : Axis::HORIZONTAL);
    }

    void Arrange() override {
        ArrangeAxis(m_Type == StackType::VERTICAL ? Axis::VERTICAL : Axis::HORIZONTAL);
    }
...
private:
    ...
    void MeasureAxis(Axis _Axis) {
        ...
        for (auto const &child : Children()) {
            ...
            if (AxisHelper::GetLayoutSizeType(child->Size(), _Axis) != SizeType::FILL) {
        ...
        for (auto const &child : Children()) {
            ...
            float orthoAvailable = AxisHelper::GetRectSize(Rect(), orthogonalAxis) -
    ...
    void ArrangeAxis(Axis _Axis) {
        ...
        for (auto const &child : Children()) {
            ...
            case SizeType::START:
                ...
                break;
            case SizeType::END:
                offset = -AxisHelper::GetPadding(Padding(), _Axis, false) - m_AlignedSize;
                break;
            case SizeType::CENTER:
                offset = AxisHelper::GetRectSize(Rect(), _Axis) / 2.0f - m_AlignedSize / 2.0f;
                break;
            default:
                break;
            }

            float position = offset + AxisHelper::GetMargin(child->Margin(), _Axis, true) +
            ...
            case SizeType::END:
                pos = AxisHelper::GetRectSize(Rect(), orthogonalAxis) -
                      AxisHelper::GetPadding(Padding(), orthogonalAxis, false) -
                      AxisHelper::GetMargin(child->Margin(), orthogonalAxis, false) - orthoSize;
                break;
            case SizeType::CENTER:
                pos = AxisHelper::GetRectSize(Rect(), orthogonalAxis) / 2.0f - orthoSize / 2.0f;
                break;
